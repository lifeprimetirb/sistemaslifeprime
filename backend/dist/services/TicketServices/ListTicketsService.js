...
const ListTicketsService = async ({ searchParam = "", pageNumber = "1", queueIds, tags, users, status, date, updatedAt, showAll, userId, withUnreadMessages, companyId, isAdmin }) => {
    let whereCondition = {
    ...
    if (showAll === "true") {
        ...
        whereCondition = { queueId: { [sequelize_1.Op.in]: queueIds } };
    }
    if (status) {
        whereCondition = {
            ...whereCondition,
