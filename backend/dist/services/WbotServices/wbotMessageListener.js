...
const handleMessage = async (msg, wbot, companyId) => {
    ...
    try {
        ...
        if (unreadMessages === 0 && whatsapp.complationMessage && (0, Mustache_1.default)(whatsapp.complationMessage, contact).trim().toLowerCase() === (lastMessage || { body: "" }).body.trim().toLowerCase()) {
            return;
        }
...
const wbotMessageListener = async (wbot, companyId) => {
    try {
        ...
        wbot.ev.on("messages.set", async (messageSet) => {
